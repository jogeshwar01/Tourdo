...
/* eslint-disable */
//to disable esLint as this is js and not nodejs so we dont want node errors
...
var displayMap = function displayMap(locations) {
  ...
  var map = new mapboxgl.Map({
    container: 'map',
    //put this on the one with id of map --thats why we put id of element as map earlier
  ...
  var bounds = new mapboxgl.LngLatBounds(); //to put markers on locations

  locations.forEach(function (loc) {
    // Create marker
    var el = document.createElement('div'); //standard JS
    ...
    .addTo(map); // Add popup on locations

    new mapboxgl.Popup({
      offset: 30 //to prevent overlap of popup on marker

    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")) //define html of popups
...
var showAlert = function showAlert(type, msg) {
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); //insert right at beginning of body ie at the top of the page
    ...
    return regeneratorRuntime.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            res = _context2.sent;
            if (res.data.status = 'success') location.reload(true); //reload the page
...
var stripe = Stripe('pk_test_51JzfKQSAJ3mr6KzPp0ThmkfhgCtYBhJYZfyOHW49xhHEy9QRF2GxaEETpkCKx6YWTpqZZDTPKMOFMBm36FXCVyxF00H5GwRxdp'); //turn on Successful payments in email in settings to get users notified on their email about the payment
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log(session); // 2) Create checkout form + charge money from credit card
            // inbuilt in stripe
  ...
  var _ref = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(e) {
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            e.preventDefault(); //just to give user feedback that password change is happening
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  var tourId = e.target.dataset.tourId; //e will be the data on button that triggered this event

  (0, _stripe.bookTour)(tourId); //we named it data-tour-id and it will be camelCased in e.target.dataset hence its name here is tourId
});
